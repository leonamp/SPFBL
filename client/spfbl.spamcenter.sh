...
if [ ! -e /etc/clamav-unofficial-sigs/user.conf ]; then
    ...
    if [[ $status_code1 == 200 && $status_code2 == 200 ]]; then
        ...
        if [[ $COUNT < 2 ]]; then
            sed -i '/^) #END ADDITIONAL DATABASES$/i \
               https://api.spam.center/download/clamav/spamcenter.ndb\
               https://api.spam.center/download/clamav/spamcenter.hsb' /etc/clamav-unofficial-sigs/user.conf
