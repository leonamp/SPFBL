...
function install() {
	...
	usermod -aG mail clamscan
        ...
        DISTRO=$(cat /etc/*-release | tr [:upper:] [:lower:] | grep -Poi '(centos-8|centos stream 8|centos-7|centos-6|ubuntu|cloudlinux 7|cloudlinux 8|almalinux-8|almalinux-9|cloudlinux server release 6|cloudLinux release 9|centos release 6|debian gnu/linux 7|debian gnu/linux 8|debian gnu/linux 10|debian gnu/linux 11)' | sort | uniq)
        if [ "$DISTRO" = "centos-7" ]; then
            ...
            wget "https://raw.githubusercontent.com/extremeshok/clamav-unofficial-sigs/master/config/os/os.centos6.conf" -O /etc/clamav-unofficial-sigs/os.conf
        elif [ "$DISTRO" = "cloudLinux release 9" ]; then
